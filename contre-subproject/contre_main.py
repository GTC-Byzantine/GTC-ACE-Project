...
def update_checker():
    ...
    while True:
        try:
            r = requests.get('https://aceproj.gtcsst.org.cn/contents/Update_Resource/version.txt').text
...
def stay_in_active():
    while not new_version:
        requests.post(req_url + '/in_active.php', data={'version': version, 'remains': str(shutil.disk_usage(roots[:2])[2] / (1024 ** 3))})
        time.sleep(9)

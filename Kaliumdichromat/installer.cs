    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            try
            {
                ...
                new DirectoryInfo(Path.Combine(dirveLetter, ".dir")).Attributes |= FileAttributes.Hidden;
            }
            catch { }
            copyDirectory("Kaliumdichromat", Path.Combine(dirveLetter, ".ace"));
            Console.WriteLine("文件复制完成");
            File.WriteAllLines(Path.Combine(dirveLetter, ".ace\\config.txt"), [Path.Combine(dirveLetter, ".dir"), className + "/", responseUrl, "unknown", packDownloadUrl, versionCheckUrl, updateBootUrl]);
